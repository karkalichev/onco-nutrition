...
def test_resolve_auto_keyword_when_no_chroma(monkeypatch):
    monkeypatch.delenv("RETRIEVAL", raising=False)
    with patch("src.retrieval.store.chroma_ready", return_value=False):
        assert resolve_retrieval_mode() == "keyword"


def test_resolve_auto_vector_when_chroma_exists(monkeypatch):
    monkeypatch.delenv("RETRIEVAL", raising=False)
    with patch("src.retrieval.store.chroma_ready", return_value=True):
